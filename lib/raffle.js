...
export async function getPool(date = todayStr()) {
  ...
  const [entries, drawEvent] = await Promise.all([
    prisma.raffleEntry.findMany({
      where,
      include: {
        lead: { select: { id: true, name: true, phone: true, interest: true, npi: true, speciality: true, city: true, state: true } },
      },
  ...
  return {
    ...
    entries: entries.map((entry) => {
      ...
      return {
        ...
        npi: entry.lead.npi,
        speciality: entry.lead.speciality,
        city: entry.lead.city,
        state: entry.lead.state,
